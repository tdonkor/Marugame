...
import { AppInitStep, AppInitStepProgressFlag, FilesLoaderService, getMainPage, MaintenanceService, PeripheralCheckProducerMessage, PosServingLocation } from 'dotsdk';
...
export class BannersComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit() {
    ...
    // disable user actions if data is loading while in service-type screen
    this.subscriptions.push(
      this.contentService.dataLoadingState.subscribe((isLoading: boolean) => this.isDataLoading = isLoading)
    );
  }
  ...
  @HostListener('click')
  public navigate() {
    if (this.blockKiosk || this.isDataLoading) {
      return;
    }
    if (this.appSettings.orderCheckInFlow === OrderCheckInFlowBundleSetting.ONLY_BANNERS) {
