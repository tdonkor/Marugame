...
import { CheckoutType } from '@dotxix/models/enums/checkout-type.enum';
...
export class GloryPaymentProgressComponent implements OnInit, OnDestroy {
  ...
  public isEndAcceptMoneyOngoing = false;
  ...
  public async finishWithOtherPayment() {
    this.isEndAcceptMoneyOngoing = true;
    ...
    this.paymentModelService.setCashAmountRefunded(endResult?.RefundAmount || 0);
    this.paymentModelService.setCurrentAmountPaidWithCash(endResult?.PaidAmount || 0);
    if (endResult && !endResult.Error) {
      ...
      if (this.paymentModelService.getAmountOwed() === 0) {
        (this.appSettingsService.posInjectionFlow === CheckoutType.PAY_AFTER_POS) ? this.router.navigate(['tender-order']) : this.router.navigate(['complete-order']) ;
      } else {
    ...
    this.subscriptions.push(
      contentRef.afterClosed.subscribe(async (response) => {
        if (response === 'Yes') {
          ...
          this.isEndAcceptMoneyOngoing = false;
        }
      })
    );
  }
  ...
  protected async startAcceptMoney() {
    ...
    let startResult;
    startResult = await AtpPaymentService.getInstance().cashStartAcceptingMoney(this.cashPaymentName).catch(() => null);;
  ...
  protected listenForPaymentProgressAndCompletion(): Subscription {
    ...
    return from(AtpPaymentService.getInstance().payProgress()).subscribe((paymentInfo: IPayProgress) => {
      ...
      if (paymentInfo.MessageClass === 'Error') {
        ...
        this.paymentModelService.setCurrentAmountPaidWithCash(paymentInfo?.CurrentPaidAmount || 0);
        if (this.isEndAcceptMoneyOngoing) {
          return;
        }
