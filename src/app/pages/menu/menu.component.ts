...
import { hasDlgMessages } from '../../helpers/dialog-message.helper';
import { CatalogAvailabilityPipe } from '@dotxix/pipes';
...
export class MenuComponent implements OnDestroy, OnInit, AfterViewChecked, AfterViewInit {
  ...
  constructor(
    ...
    protected catalogButtonAvailability: CatalogAvailabilityPipe,
  ...
  public select(button: DotButton) {
    ...
    if (
      ...
      if (!button.Page && hasDlgMessages(button)) {
        const contentRef = this.dynamicContentService.openContent(ConfirmDialogComponent, {
          button,
          leftButtonText: this.translatePipe.transform('23'),
          rightButtonText: this.translatePipe.transform('28'),
  ...
  public unavailableButton(button: DotButton) {
    if (button.hasCombos) {
      ...
      return this.isButtonStatusUnavailable(button);
    }
  }
  ...
  private openDropdown(button: DotButton) {
    ...
    const index = this.catalogButtonAvailability.transform(this.page.Buttons)?.findIndex((btn) => btn.Link === button.Link);
    if (index >= 0) {
      this.dropdownPage = {
        display: true,
