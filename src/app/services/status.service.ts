...
export class StatusService {
  ...
  public isKioskBlockedDueToPeripherals(value: PeripheralCheckProducerMessage): boolean {
    ...
    const isNonPaymentPeripheralsBlocking = value.entries
    ...
    const isAtLeastOneEnabledPaymentPeripheralAvailable = paymentEntries.some((paymentEntry) => {
      return enabledPaymentTypes.some(
        (enabledPaymentType) => paymentEntry.peripheralSubtype === enabledPaymentType.PaymentType && !paymentEntry.metadata.block
      );
    });
  ...
  public getKioskStatusColor(value: PeripheralCheckProducerMessage): KioskStatusColor {
    const isCashAvailable = value.entries.some(
      (p) => p.peripheralType === 'Payment' && p.peripheralSubtype === PAYMENT_TYPE.CASH && !p.error
    );
    const isCashEnabledInBS = this.appSettings.paymentTypes.some((p) => p.PaymentType === PAYMENT_TYPE.CASH && p.PaymentIsEnabled);
    const isCashInventoryWarn = value.entries.some(
      (p) =>
        p.peripheralType === 'Payment' &&
        p.peripheralSubtype === PAYMENT_TYPE.CASH &&
        p.metadata.inventoryStatus.overallStatus === CashInventoryOverallStatus.WARNING
    );
    ...
    const isCardAvailable = value.entries.some(
      (p) => p.peripheralType === 'Payment' && p.peripheralSubtype === PAYMENT_TYPE.CARD && !p.error
    );
   ...
   * @param entries PeripheralCheckEntry
   */
  private mutateCashPeripheralsBasedOnInventoryStatus(entries: PeripheralCheckEntry[]) {
    entries
      .filter((entry) => entry.peripheralType === 'Payment' && entry.peripheralSubtype === PAYMENT_TYPE.CASH && !entry.error)
      .forEach((entry) => {
        ...
        if ((isDispensableInventoryEmpty && !this.appSettings.ignoreGloryDispensableEmptyStatus) || isInternalInventoryFull) {
          entry.metadata.inventoryStatus.overallStatus = CashInventoryOverallStatus.NOT_OK;
        } else if (isDispensableInventoryNearEmpty || isInternalInventoryNearFull) {
          entry.metadata.inventoryStatus.overallStatus = CashInventoryOverallStatus.WARNING;
        } else {
          entry.metadata.inventoryStatus.overallStatus = CashInventoryOverallStatus.OK;
        }
  ...
  private getAvailablePayments(value: PeripheralCheckProducerMessage) {
    ...
    const cardStatus = this.appSettings.paymentTypes.find((p) => {
      return (
        p.PaymentType === PAYMENT_TYPE.CARD &&
        p.PaymentIsEnabled &&
        value.entries.find((q) => {
    ...
    const cashStatus = this.appSettings.paymentTypes.find((p) => {
      return (
        p.PaymentType === PAYMENT_TYPE.CASH &&
        p.PaymentIsEnabled &&
        value.entries.find((q) => {
