...
export class ButtonModifierComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  @Input() public isSuggestive = false;
  ...
  public onQuantityUpdate(count: 1 | -1): void {
    if (!this.isBucketStandard && !this.isComplementButton && !this.modifierButton.hasCombos && !this.modifierButton.hasModifiers && !this.isSuggestive) {
      if (!this.hasPrefixes && !this.hasDefaultQuantity) {
        if (count > 0 && this.maxQuantityGroup && this.getQuantityButtons + 1 >= this.maxQuantityGroup) {
          this.quantityChanged.emit(count);
        } else if (count > 0 && this.getQuantityButtons < this.maxQuantityGroup) {
