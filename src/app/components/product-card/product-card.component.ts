...
export class ProductCardComponent implements OnInit, OnDestroy {
  ...
  public buttonTranslatedCaption: string = '';
  public buttonTranslatedPicture: string = '';
  ...
  public ngOnInit(): void {
    this.buttonTranslatedCaption = this.translateButtonCaption();
    this.buttonTranslatedPicture = this.translateButtonPicture();
  }
  ...
  public changeProduct(): void {
    this.basketButton.isChanged = true;
    if (this.basketButton.hasModifiers) {
      this.dynamicContentService.openContent(ButtonDetailsComponent, { btn: this.basketButton });
    }

    if (this.basketButton.hasCombos) {
      this.dynamicContentService.openContent(ComboStepperComponent, { btn: this.basketButton });
    }
  }
  ...
  private translateButtonPicture(): string {
    if (this.basketButton.hasCombos) {
      return this.translatePicturePipe.transform(this.parentButton);
    }

    const translateCatalog = this.translateCatalogPicturePipe.transform(this.basketButton);
  ...
  private translateButtonCaption(): string {
    // TODO: Combos are kept inside combosCatalog.json. Need to
    if (this.basketButton.hasCombos) {
      const title = this.translationsService.getTranslatedButtonCaption(this.parentButton);
      if (title === null) {
        return this.translationsService.getTranslatedButtonCaption(this.basketButton);
      }
  ...
  private removeButton() {
    if (this.basketButton.isPromo && !!this.basketButton['Promo']) {
      const promoUUID = this.basketButton['Promo'].UUID;
      if (!!this.basketButton['$$OrderDiscount']) {
      ...
      this.basketService.removeButtonFromBasket(this.basketButton, true);
    }
  }

  private replaceCaption(): string {
    const title = this.dotTranslatePipe.transform('2020122102');
    const caption = this.translateButtonCaption();
