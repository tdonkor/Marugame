...
export class ButtonModifierListComponent implements OnInit, AfterViewChecked {
  ...
  public selectModifiers(button: DotButton, mod: DotModifier, index: number) {
    ...
    if (this.displayText && !this.isBucketStandard) {
  ...
  public bucketStandard(changedQuantity: number, btn: DotButton, mod: DotModifier, index: number) {
    if (changedQuantity > 0) {
      this.selectModifiers(btn, mod, index);
    } else {
      btn.quantity--;
      if (btn.quantity === 0) {
        btn.Selected = false;
